...
const createBoxes = (size) => {
    ...
    document.querySelector(
        ".box-container"
    ).style.gridTemplateColumns = `repeat(${size}, var(--box-size))`;
};
...
const open_setting = () => {
    ...
    document.querySelector(".word-length").innerText =
...
function changeBoxColor() {
    ...
    const previousBoxes = [...boxes].slice(
        guessCount * word_size - word_size,
        currentActiveIndex
    );
...
const startTimer = () => {
    ...
    interval = setInterval(() => {
        ...
        timer.style.background = `conic-gradient(rgb(236, 181, 15) ${
            (60 - countdown) * 6
        }deg, rgb(47, 45, 45) 0deg)`;
